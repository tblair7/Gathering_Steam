...
def get(appid, printProgress=True):
    ...
    **progress** -- Set to true to print the progress of each request.
    ...
    api_key_list = [line.rstrip('\n') for line in open('steam_api_key.txt')]
    api_key = ''.join(api_key_list)
    ...
    def _makeRequest(appid, params):
        ...
        **params** -- An object used to build the Steam API query. (https://partner.steamgames.com/doc/store/getreviews)
        '''
        response = requests.get(url=ENDPOINT+appid+key_string, params=params) # get the data from the endpoint
    ...
    params = {
        'json': 1,
        'filter': 'updated', # sort by: recent, updated
